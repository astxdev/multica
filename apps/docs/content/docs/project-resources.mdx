...
A local directory has two execution modes, set per resource with `execution_mode`.
...
Requires the directory to be a git repository with at least one commit. If it is not, tasks fail with an explicit error rather than quietly falling back to serial execution. The runtime on that machine must also be new enough to implement the mode: saving the resource is refused with an upgrade hint while the daemon is too old, and a daemon that encounters a mode it does not recognise fails the task rather than running in place — asking for `worktree` is asking for isolation, so silently editing your working copy instead is never the fallback.
...
- **The result is a branch in your repo**, named `agent/<agent>/<task>`. Find it with `git branch`, review it with `git log` / `git diff`, and merge or cherry-pick it yourself. Multica never merges it for you.
